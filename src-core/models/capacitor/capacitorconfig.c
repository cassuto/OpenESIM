...
int
LIB_FUNC(capacitor_config)( circ_element_t *element, int op, ... )
{
    ...
    case ELM_CONFIG_LIST_PROP:  /* Query the parameter property list */
      {
        ...
        static model_variable_prop_t prop_list[] =
            {
              { "c", "Capacitance value", MDEL_VAR_DOUBLE }
            };
        *prop = prop_list;
      }
      break;
    ...
    case ELM_CONFIG_GET:  /* Query the value of parameter */
      {
        ...
        switch ( param_id )
        {
          case 0: *(va_arg( vlist, double* )) = param->cap; break;
          default: rc = -DS_FAULT;
        }
      }
      break;
    ...
    case ELM_CONFIG_SET:  /* Set the value of parameter and make it effective */
      {
        ...
        switch ( param_id )
        {
          case 0: /* Set Capacitance value */
            {
              double val = va_arg( vlist, double );
              param->cap = val;
              rc = set_resist( element, param, param->dT/val );
              break;
            }
          default:
            rc = -DS_FAULT;
