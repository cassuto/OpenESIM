...
DeviceLibraryEntry *dev_bjt_npn::libraryEntry()
{
  return new DeviceLibraryEntry(
      ...
      /* manufacturer */    "BJT",
      /* symbolfile */      "bjt_npn.ssym",
      dev_bjt_npn::construct,
      0l);
}

IDevice *dev_bjt_npn::construct( const char *reference, int id, void *reserved )
{
  return new dev_bjt_npn( reference, id, reserved );
}

dev_bjt_npn::dev_bjt_npn( const char *reference, int id, void *reserved )
  : IDevice( reference, id, reserved )
{}

int dev_bjt_npn::create( ISchematic *schematic, IPropertyContainer *properties )
{
...
int dev_bjt_npn::init( ISchematic *, IPropertyContainer *properties )
{
  int rc = circ_element_config(( m_mdel, ELM_CONFIG_SET, 1/*pnp*/, (int)false )); UPDATE_RC(rc);
      rc = properties->configDevice( this );
  return rc;
}

int dev_bjt_npn::config( int op, ... )
{
  int rc = 0;
  va_list vlist;
  va_start( vlist, op );
