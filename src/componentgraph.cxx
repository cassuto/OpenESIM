...
{
  if( !MainWindow::instance()->templatestyle()->isStyle( style ) )
    return;

  StyleItem s = MainWindow::instance()->templatestyle()->lineStyle( style, m_selected );

  if( s.line != LINE_NONE )
    this->setPenColor( s.color.r, s.color.g, s.color.b );
  this->setPenWidth( s.width );
  this->setPenStyle( s.line );

  s = MainWindow::instance()->templatestyle()->fillStyle( style, m_selected );

  if( s.brush != BRUSH_NONE )
    this->setBrushColor( s.color.r, s.color.g, s.color.b );
  this->setBrushStyle( s.brush );
}
...
void ComponentGraph::setBrushStyle( BrushStyle style )
{
  ...
  switch( style )
  {
    case BRUSH_NONE: brush.setStyle( Qt::NoBrush ); break;
    case BRUSH_SOLID: brush.setStyle( Qt::SolidPattern ); break;
    case BRUSH_HOR: brush.setStyle( Qt::HorPattern ); break;
    case BRUSH_VER: brush.setStyle( Qt::VerPattern ); break;
    case BRUSH_CROSS: brush.setStyle( Qt::CrossPattern ); break;
    default: return;
  }
