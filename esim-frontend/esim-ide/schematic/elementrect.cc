...
ElementRect::ElementRect(int x1, int y1, int x2, int y2) :
    Element(),
    m_x1(x1), m_y1(y1),
    m_x2(x2), m_y2(y2),
...
const char *ElementRect::classname() const
{
  return "rect";
}

int ElementRect::locationType() const
{
  return S0_POS_4P;
}

void ElementRect::locate(int opcode, int paramSize, ...)
{
  va_list vlist;
...
void ElementRect::render(RenderDevice *device)
{
  ...
  device->rect(cx1, cy1, cx2-cx1+1, cy2-cy1+1);

  device->setFillStyle(oldfill);
}

void ElementRect::bounding(int *x1, int *y1, int *x2, int *y2) const
{
  *x1 = s_min(m_x1, m_x2); *y1 = s_min(m_y1, m_y2);
  *x2 = s_max(m_x1, m_x2), *y2 = s_max(m_y1, m_y2);
}

int ElementRect::serialize(persistence::PersistenceDom *dom)
{
...
int ElementRect::deserialize(persistence::PersistenceDom *dom)
{
